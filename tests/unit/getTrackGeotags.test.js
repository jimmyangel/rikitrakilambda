...
test("returns 404 if no trackPhotos", async () => {
  ddbMock.on(GetCommand).resolves({ Item: { trackId: "S1o8dr6j0", trackPhotos: [] } })
...
test("handles DynamoDB error gracefully", async () => {
  ...
  ddbMock.on(GetCommand).rejects(new Error("DDB failure"))
